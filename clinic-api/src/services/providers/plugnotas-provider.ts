...
export class PlugNotasProvider implements NFSeProvider {
    ...
    async validateCertificate(certificate: Buffer, password: string): Promise<CertificateValidation> {
        try {
            // PlugNotas doesn't have a certificate validation endpoint
            // We'll do basic certificate parsing locally instead
            console.log('PlugNotas: Certificate validation skipped - no API endpoint available');

            return {
                isValid: true, // We assume it's valid since we can't check with PlugNotas
                error: undefined
            };
        } catch (error) {
